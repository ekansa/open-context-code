...
class OpenContext_SolrIndexer {
	...
	public static function reindex_item($spatialItem, $about_oc_URI = false){
		...
		if($about_oc_URI){
			...
			$spatialItem->registerXPathNamespace("oc", OpenContext_OCConfig::get_namespace("db_oc", "spatial"));
		}
		...
		*/
		...
		$solrDocument->item_type = "spatialunit";
		...
		if ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property")) {
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='nominal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_nom) {
					$solrDocument->setMultiValue("variable_NOB", $var_nom);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_nom) . "_var_NOB_val";
					$solrDocument->setMultiValue($dynamic_field_name, $show_val);
				}
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='ordinal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_ord) {
					$solrDocument->setMultiValue("variable_NOB", $var_ord);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_ord) . "_var_NOB_val"; 
					$solrDocument->setMultiValue($dynamic_field_name, $show_val);
				}
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='boolean']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_bool) {
					$solrDocument->setMultiValue("variable_NOB", $var_bool);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_bool) . "_var_NOB_val";  // this escaping is probably not necessary but could help handle messy data
					$solrDocument->setMultiValue($dynamic_field_name, $show_val);
				}
		...
		*/
		...
		foreach ($spatialItem->xpath("//arch:spatialUnit/oc:metadata/oc:geo_reference") as $geo_reference) {
			foreach ($geo_reference->xpath("oc:geo_lat") as $geo_lat) {
				$solrDocument->geo_lat = (string)$geo_lat;
				//echo "geo_lat : " . $geo_lat;
				//echo "<br/>";
			}
			foreach ($geo_reference->xpath("oc:geo_long") as $geo_long) {
				$solrDocument->geo_long = (string)$geo_long;
	...
	public static function reindex_resource($mediaItem, $spatialItem){
		
		// increase the memory limit
		ini_set("memory_limit", "2048M");
	
		$spatial_prefix = "sp___"; //prefix to differentiate values from spatialunits, where there may be overlap with media
		$media_var_prefix ="im___";
		...
		*/
		...
		$solrDocument->uuid = $media_uuid; // add it to the solr document
	
		//get item types
		foreach($mediaItem->xpath("//arch:resource/@type") as $media_type) {
		    $media_type = strtolower($media_type);
		}
		$solrDocument->item_type = $media_type; // add it to the solr document
		...
		foreach ($mediaItem->xpath("//arch:resource/arch:name/arch:string") as $item_label) {
			...
			*/
			...
			foreach($mediaItem->xpath("//arch:resource/@ownedBy") as $projid) {
				$solrDocument->project_id = $projid."";
			}
		...
		// Count image media <field name="image_media_count" type="sint" indexed="true" stored="false" required="true" multiValued="false" />
		$image_media = $mediaItem->xpath("//arch:resource/arch:links/oc:media_links/oc:link[oc:type='image']");  
		// if the xpath above returns either false or null... 
		if (!$image_media) {
			$image_media_count = 0;
			$solrDocument->image_media_count = 0;
		...
		// Count other binary media <field name="other_binary_media_count" type="sint" indexed="true" stored="false" required="true" multiValued="false" />
		$other_binary_media = $mediaItem->xpath("//arch:resource/arch:links/oc:media_links/oc:link[oc:type!='image']");
		if (!$other_binary_media) { // if there are no other binary media, set the count to 0.
			$other_binary_media_count = 0;
			$solrDocument->other_binary_media_count = 0;
		} else { // otherwise, the xpath above returns a non-empty array which we can count.
		...
		// Count diary items <field name="diary_count" type="sint" indexed="true" stored="false" required="true" multiValued="false" />
		$diary = $mediaItem->xpath("//arch:resource/arch:links/oc:diary_links/oc:link");
		if (!$diary) {
			$diary_count = 0;
			$solrDocument->diary_count = 0;
		} else { // otherwise, the xpath above returns a non-empty array which we can count.
		...
		if ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:user_tags/oc:tag")) {
			foreach ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:user_tags/oc:tag[@type='text' and @status='private']") as $tag) { // loop through and get the tags. 
		...
		*/
		...
		// Properties and variables (from spatial item)
		$number_properties = 0;  // used to help calculate the interest_score. if the item has no properties, set $number_properties to 0
		// Verify that there are properties associated with this item.
		if ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property")) {
			// get property count for the interest_score
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='alphanumeric']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_alpha) {
					//$note = $var_alpha . " ";
					$var_alpha = $spatial_prefix.$var_alpha;
					$solrDocument->setMultiValue("variable_alpha", $var_alpha);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$note .= $show_val;
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='nominal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_nom) {
					$var_nom = $spatial_prefix.$var_nom;
					$solrDocument->setMultiValue("variable_NOB", $var_nom);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_nom) . "_var_NOB_val";
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='calendar']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_cal) {
					$var_cal = $spatial_prefix.$var_cal;
					$solrDocument->setMultiValue("variable_calendric", $var_cal);
				}
				foreach ($var_label->xpath("../date") as $cal_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_cal) . "_var_cal_val";
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='integer']") as $var_label) {
				foreach ($var_label->xpath("../arch:integer") as $show_val) {
					...
					if ($show_val && (intval($show_val) === $show_val)) {
						...
						// $show_val is non-null and is an integer
						$var_label = $spatial_prefix.$var_label;
						$solrDocument->setMultiValue("variable_integer", $var_label);
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='ordinal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_ord) {
					$var_ord = $spatial_prefix.$var_ord;
					$solrDocument->setMultiValue("variable_NOB", $var_ord);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_ord) . "_var_NOB_val"; 
			...
			foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:properties/arch:property/oc:var_label[@type='boolean']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_bool) {
					$var_bool = $spatial_prefix.$var_bool;
					$solrDocument->setMultiValue("variable_NOB", $var_bool);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					$dynamic_field_name = OpenContext_SolrIndexer::solrEscape($var_bool) . "_var_NOB_val";  // this escaping is probably not necessary but could help handle messy data
		...
		//get properties from the media item
		if ($mediaItem->xpath("//arch:resource/arch:properties/arch:property")) {
			// get property count for the interest_score
			$number_properties = count($mediaItem->xpath("//arch:resource/arch:properties/arch:property"));
			// get the alphanumeric properties and index them as a 'notes' field. 
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='alphanumeric']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_alpha) {
					$note = $var_alpha . " ";
					$solrDocument->setMultiValue("variable_alpha", $var_alpha);
				}
			...
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='nominal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_nom) {
					$solrDocument->setMultiValue("variable_NOB", $var_nom);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
			...
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='calendar']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_cal) {
					$solrDocument->setMultiValue("variable_calendric", $var_cal);
				}
				foreach ($var_label->xpath("../date") as $cal_val) {
			...
			// check for integer variables 
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='integer']") as $var_label) {
				foreach ($var_label->xpath("../arch:integer") as $show_val) {
					// test whether $show_val's value is non-null and really is an integer. It may be neither.
					if ($show_val && (intval($show_val) === $show_val)) {
						// $show_val is non-null and is an integer
			...
			// get the decimal variable
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='decimal']") as $var_label) {
				foreach ($var_label->xpath("../arch:decimal") as $show_val) {
					// make sure the value is numeric (so it can be indexed as a float).
					if (is_numeric((string) ($show_val))) {
						$solrDocument->setMultiValue("variable_decimal", $var_label);
			...
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='ordinal']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_ord) {
					$solrDocument->setMultiValue("variable_NOB", $var_ord);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
			...
			foreach ($mediaItem->xpath("//arch:resource/arch:properties/arch:property/oc:var_label[@type='boolean']") as $var_label) {
				foreach ($var_label->xpath(".") as $var_bool) {
					$solrDocument->setMultiValue("variable_NOB", $var_bool);
				}
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
		...
		if(strtolower($media_type)== "image"){
			...
			if(($pixels > 2000000)&&($pixels <= 3999999)){
				$image_size = "Large";
			}
			if(($pixels > 750000)&&($pixels <= 1999999)){
				$image_size = "Medium";
			}
			if(($pixels > 40000)&&($pixels <= 649999)){
				$image_size = "Small";
			}
			if(($pixels > 1)&&($pixels <= 39999)){
				$image_size = "Tiny";
			}
			
			$solrDocument->setMultiValue("variable_NOB", $media_var_prefix."Image Size");
			$solrDocument->setMultiValue($media_var_prefix.OpenContext_SolrIndexer::solrEscape("Image Size")."_var_NOB_val", $image_size);
			
			foreach ($mediaItem->xpath("//arch:externalFileInfo/arch:thumbnailURI") as $image_uri){
				$thumb_media_uri = $image_uri."";
			}
			foreach ($mediaItem->xpath("//arch:externalFileInfo/arch:previewURI") as $image_uri){
				$prev_media_uri = $image_uri."";
			}
		...
		foreach ($spatialItem->xpath("//arch:spatialUnit/arch:observations/arch:observation/arch:links/oc:person_links/oc:link") as $person_link) {
			foreach ($person_link->xpath("oc:name") as $person_name) {
				$solrDocument->setMultiValue("person_link", $person_name);
				$person_name = $spatial_prefix.$person_name;
				//echo "person_link: " . $person_name;
				//echo "<br/>";
			}
			foreach ($person_link->xpath("oc:relation") as $relation) {
		...
		if($mediaItem->xpath("//arch:resource/arch:links/oc:person_links/oc:link")){
			foreach ($mediaItem->xpath("//arch:resource/arch:links/oc:person_links/oc:link") as $person_link) {
				foreach ($person_link->xpath("oc:name") as $person_name) {
					$solrDocument->setMultiValue("person_link", $person_name);
					//echo "person_link: " . $person_name;
					//echo "<br/>";
		...
		*/
		...
		$count_ext_refs = 0; // used to help calculate interest_score
		if ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:external_references")) {		
			foreach ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:external_references/oc:reference/oc:name") as $external_reference) {
				$solrDocument->setMultiValue("external_reference", $external_reference);
				//echo "external_reference: " . $external_reference;
				//echo "<br/>";
				$count_ext_refs++;  // used to help calculate interest_score
		...
		*/
		...
		if ($mediaItem->xpath("//arch:notes/arch:note")) {
			foreach ($mediaItem->xpath("//arch:notes/arch:note/arch:string") as $note) {
				$solrDocument->setMultiValue("notes", $note);
				//echo "notes: " . $note;
				//echo "<br/>";
			} 
		...
		/* Note: view_count removed from index in favor of interest_score.
		 * Now used to help calculate the interest_score */
		foreach ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:item_views/oc:count") as $item_views) {
			//echo "view_count: " . $item_views;
			//echo "<br/>";
		}
		...
		*/
		...
		if ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:user_tags/oc:tag")) {
			foreach ($mediaItem->xpath("//arch:resource/oc:social_usage/oc:user_tags/oc:tag[@type='chronological' and @status='private']") as $private_chrono_tag) {
				foreach ($private_chrono_tag->xpath("oc:chrono/oc:time_start") as $private_time_start) {
					$solrDocument->setMultiValue("private_time_start", $private_time_start);
					//echo "private_time_start: " . $private_time_start;
					//echo "<br>";
		...
		*/
		...
		foreach ($mediaItem->xpath("//arch:resource/oc:metadata/dc:subject") as $subject) {
			$solrDocument->setMultiValue("subject", $subject);
			//echo "subject: " . $subject;
			//echo "<br/>";
		}
		...
		<field name=" coverage " type=" string " indexed=" true " stored=" true " required=" false " multiValued=" true " />
		 */
		foreach ($mediaItem->xpath("//arch:resource/oc:metadata/dc:coverage") as $coverage) {
			$solrDocument->setMultiValue("coverage", $coverage);
			//echo "coverage: " . $coverage;
			//echo "<br/>";
		}
		...
		<field name=" license_uri " type=" string " indexed=" true " stored=" true " required=" true " multiValued=" false " />
		*/
		foreach ($mediaItem->xpath("//arch:resource/oc:metadata/oc:copyright_lic/oc:lic_URI") as $license_uri) {
			$solrDocument->license_uri = $license_uri;
			//echo "license_uri: " . $license_uri;
			//echo "<br/>";
		}
		...
		foreach ($mediaItem->xpath("//atom:entry") as $entry) {
			$full_entry_string = $entry->asXML();
		}
		
		foreach ($mediaItem->xpath("//atom:entry/arch:resource") as $media_xml) {
			$media_string = $media_xml->asXML();
		}
		
		//remove full archaeoml record from the abbreviated atom entry
		$atom_entry = str_replace($media_string, "", $full_entry_string);
		...
		$link_to_preview = "";
		if(strtolower($media_type) == "image"){
			$link_to_preview ="<p>The preview images is: <a href='".$prev_media_uri."' class='img_preview_link'>(here)</a></p>";
		}
		
		$atom_entry = str_replace('<h1>Media Resource Preview</h1>', '<h2>Media Resource: '.$item_label.'</h2>', $atom_entry);
		$atom_entry = str_replace('title="Media preview"', 'title="Thumbnail"', $atom_entry);
		$atom_entry = str_replace('src="'.$prev_media_uri.'"', 'src="'.$thumb_media_uri.'"', $atom_entry);
		$atom_entry = str_replace('</div>', $link_to_preview.(chr(13)).'</div>', $atom_entry);
		...
		$atomFullDocString = $mediaItem->saveXML();
		$solrDocument->atom_full = $atomFullDocString;
