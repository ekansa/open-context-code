...
class OpenContext_SolrIndexer {
	...
	public static function update_view_count($spatialItem){
		...
		// Register OpenContext's namespace
		$spatialItem->registerXPathNamespace("oc", OpenContext_OCConfig::get_namespace("oc", "spatial"));
		$spatialItem->registerXPathNamespace("default", OpenContext_OCConfig::get_namespace("atom"));
	...
	public static function reindex_item($spatialItem){
		...
		$propArray = OpenContext_SolrIndexer::AddProperties($solrDocument, $spatialItem, "space");
		//return array("solr"=>$solrDocument, "numProps" =>$number_properties, "note"=>$note);
		$solrDocument = $propArray["solr"];
		$number_properties = $propArray["numProps"];
		$note = $propArray["note"];
		...
		*/
		...
		$solrDocument->atom_full = "";
		$solrDocument->atom_abbreviated = "";
	...
	public static function reindex_resource($mediaItem, $spatialItem){
		...
		// Register OpenContext's namespace
		$mediaItem->registerXPathNamespace("oc", OpenContext_OCConfig::get_namespace("oc", "media"));
		...
		// Register the Atom namespace
		//$mediaItem->registerXPathNamespace("atom", OpenContext_OCConfig::get_namespace("atom"));
		...
		//process properties for the related spatial item
		...
		$propArray = OpenContext_SolrIndexer::AddProperties($solrDocument, $mediaItem, "resource");
		//return array("solr"=>$solrDocument, "numProps" =>$number_properties, "note"=>$note);
		$solrDocument = $propArray["solr"];
		$number_properties += $propArray["numProps"];
		$note .= $propArray["note"];
		...
		$thumb_media_uri = false;
		$prev_media_uri = false;
	...
	//this adds properties to Solr
	public static function AddProperties($solrDocument, $xmlItem, $itemType = "space"){
		...
		if ($xmlItem->xpath($xpathPrefix."/arch:properties/arch:property")) {
			...
			$number_properties = count($xmlItem->xpath($xpathPrefix."/arch:properties/arch:property"));
			...
			foreach ($xmlItem->xpath($xpathPrefix."/arch:properties/arch:property/oc:var_label[@type='alphanumeric']") as $var_label) {
				...
				foreach ($var_label->xpath("../oc:show_val") as $show_val) {
					//$note .= $show_val;
					//$show_val = OpenContext_SolrIndexer::solrEscape($show_val);
					$show_val = htmlspecialchars($show_val, ENT_NOQUOTES, 'UTF-8'); 
