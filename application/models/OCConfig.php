...
class OpenContext_OCConfig {
	...
	const username = 'opencontext'; // database username
	const password = 'swt1kmk2'; //database username password
	//const dbname = 'oc_working'; //database name
	const dbname = 'oc_working_new'; //database name
	...
	const updateCacheLife = 720000; //lifetime of the last update check
	...
	public static function get_db_config(){
       ...
       public static function last_update($type = "space"){
		...
		$sql = "SELECT MAX(space.created) AS maxupdated
		FROM space
		";
		
		if($type == "media"){
			$cache_id = self::MediaUpdateCache;
			$sql = "SELECT MAX(resource.updated) AS maxupdated
			FROM resource
			";
		}
