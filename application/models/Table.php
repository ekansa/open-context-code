...
class Table {
	...
	if(isset($tableData["meta"]["TabCreated"])){
            $value = date("Y-m-d\TH:i:s\Z", strtotime($tableData["meta"]["TabCreated"]));
	    //$OpenContextItem->addProperty("Table Created Time", false, 'calendar'); //add the variable name as top level prop
	    //$OpenContextItem->addProperty($value, array("Table Created Time"), 'calendar'); //add the value for the prop
            //$OpenContextItem->addfullPropertyPath( array("Table Created Time", $value) ); //add taxonomy path
        }
