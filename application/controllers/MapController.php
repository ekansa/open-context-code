...
class mapController extends Zend_Controller_Action {
    ...
    */
    ...
    public function indexAction() {
        ...
        OpenContext_SocialTracking::update_referring_link('map', $this->_request->getRequestUri(), @$_SERVER['HTTP_USER_AGENT'], @$_SERVER['HTTP_REFERER']);
        
        $this->_helper->viewRenderer->setNoRender();
        $jsonString = file_get_contents("http://opencontext.org/sets/.json?cat=Animal+Bone");
        //$jsonObject = json_decode($jsonString, true);
        $jsonObject = Zend_Json::decode($jsonString);
        //echo var_dump($jsonObject);
        foreach($jsonObject["facets"] as $type => $actType){
            echo "<br/><br/><strong>$type:</strong>"; 
            foreach($actType as $actFacet){
                echo "<p><a href='".$actFacet["href"]."'>".$actFacet["href"]."</a> (".$actFacet["name"].")</p>";
            }
        }
    ...
    public function mapjsonAction(){
        ...
        $atomFacetURI = str_replace(".json", ".atom", $host.$requestURI);
        $atomFacetURI = str_replace( $host."/map/details",  $host."/sets/facets", $atomFacetURI);
        $facetObj = OpenContext_FacetAtom::atom_to_object($atomFacetURI);
        $itemsArray = OpenContext_FacetAtom::facet_object_timemap($facetObj);
        
        //echo $atomFacetURI;
        //echo Zend_Json::encode($itemsArray);
        $jsonString = file_get_contents("http://opencontext.org/sets/.json?cat=Animal+Bone");
        $jsonObject = json_encode($jsonString);
        foreach($jsonObject["facets"] as $actFacet){
            echo "<br/><a href='".$actFacet["href"]."'>".$actFacet["href"]."</a> (".$actFacet["name"].")";
        }
    ...
    public function startjsonAction(){
        $this->_helper->viewRenderer->setNoRender();
        $host = OpenContext_OCConfig::get_host_config();
