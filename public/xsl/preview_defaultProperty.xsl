...
<div id="item_name" class="subHeader">Descriptive Property: <span class="bodyText"><xsl:value-of select="arch:property/arch:name/arch:string"/></span></div>       
...
<div id="left_des">
	...
	<div id="allnotes" class="bodyText">
		...
		<p class="bodyText">Within the context of the "<xsl:value-of select="arch:property/oc:metadata/oc:project_name"/>" project, the variable <em><xsl:value-of select="//oc:manage_info/oc:propVariable"/></em> has <xsl:value-of select="//oc:numUniqueVals"/> unique values. <xsl:if test="//oc:propMean" >The average value is: <xsl:value-of select="//oc:propMean"/></xsl:if><xsl:if test="//oc:freqRank" >The value <em><xsl:value-of select="arch:property/arch:name/arch:string"/></em> is ranked number <strong><xsl:value-of select="//oc:freqRank"/></strong> in frequency.</xsl:if></p>
		<br/>
...
<div id="right_des">
	...
	<div id="all_media" class="bodyText" align="left">
		<p class="subHeader">Linked Media  (<xsl:value-of select="count(descendant::arch:property/arch:observations/arch:observation/arch:links/oc:media_links/oc:link)"/> files)</p>
		<xsl:if test="count(descendant::arch:property/arch:observations/arch:observation/arch:links/oc:media_links/oc:link) != 0" >
			<table border="0" cellpadding="1">
				<xsl:for-each select="arch:property/arch:observations/arch:observation/arch:links/oc:media_links/oc:link">
						<tr>
							<td>
								<a>
