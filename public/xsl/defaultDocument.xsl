...
<xsl:attribute name="href">http://opencontext.org/document/<xsl:value-of select="arch:resource/@UUID"/>.atom</xsl:attribute>
</link>
...
<div id="left_des">
	...
	<xsl:if test="count(descendant::arch:resource/arch:links/oc:space_links/oc:link) != 0" >
		<div id="all_links">
			<p class="subHeader">Linked Items (<xsl:value-of select="count(descendant::arch:resource/arch:links/oc:space_links/oc:link)"/> items)</p>
		...
		<p class="subHeader">Descriptive Tags  (<xsl:value-of select="count(descendant::arch:resource/oc:social_usage/oc:user_tags/oc:tag[@status='public'])"/>)</p>
			<p class="bodyText">
				<xsl:for-each select="//oc:social_usage/oc:user_tags/oc:tag[@status='public']">
				   <a>
						<xsl:if test="@type != 'chronological'"><xsl:attribute name="href">../search/?doctype=document&amp;tag[]=<xsl:value-of select="oc:name"/></xsl:attribute></xsl:if>
						<xsl:if test="@type = 'chronological'"><xsl:attribute name="href">../search/?doctype=document&amp;t-start=<xsl:value-of select="//oc:time_start"/>&amp;t-end=<xsl:value-of select="//oc:time_finish"/></xsl:attribute></xsl:if><xsl:value-of select="oc:name"/></a><xsl:if test="position() != last()"> , </xsl:if>
				</xsl:for-each>
			</p>
			<xsl:if test="//oc:user_tags/oc:tag[@type = 'chronological']">
						<p class="tinyText"><strong>Editor's Note:</strong> Date ranges for asociated contexts and finds are approximate and do not necessarily reflect the opinion of data contributors. These dates are provided only to facilitate searches.</p>
				</xsl:if>
		<br/>	
	</div>
