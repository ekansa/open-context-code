...
class OaiPmhRepository_Metadata_OaiDc extends OaiPmhRepository_Metadata_Abstract
{
    ...
    public function appendMetadata() 
    {
        ...
        {   
            $dcElements = $this->item->dc_metadata;
             //cludge for some reason needed to insure no duplicate metadata
            
            foreach($dcElements as $ActElement)
            {
                $item_element_name = $ActElement->element;
                $item_element_value = $ActElement->value;
                
                if(($this->item->collection_id != "projects")&&($item_element_name == "title")){
                    $item_element_value = str_replace("(Overview)", " ".$this->item->collection_id, $item_element_value);
                }
                
                if($elementName == $item_element_name){
                    $this->appendNewElement($oai_dc, 
                        'dc:'.$item_element_name, $item_element_value);
                }
            }
