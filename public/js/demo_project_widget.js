function handleCallBack(data,options){
 ...
 //setting widget container div style
	...
	var contextNum = contexts.length;
	var testBegin = 0;
	var testEnd = 0;
	
	for (var iii = 0; iii < contextNum; iii++){
	    
	    testBegin = parseInt(contexts[iii]['timespan']['begin']);
	    testEnd = parseInt(contexts[iii]['timespan']['end']);
	    
	    if(testBegin < timeBegin){
		timeBegin = testBegin;
	    }
	    if(testEnd > timeEnd){
		timeEnd = testEnd;
   ...
   widget_div.innerHTML+=recordTypes;
 	...
 	for (var i=0; i<media['length']; i++){
 		mediaContributed+='<li><a href="'+media[i]['href']+'">Records with '+media[i]['name']+': '+media[i]['item_count']+'</a></li>';
 	}
 	...
 	for (var i = 0; i<k; i++){
		contributors += '<li><a href="'+data['main_people'][i]['href']+'" title="View '+ data['main_people'][i]['name'] +'\'s OpenContext profile" >' + data['main_people'][i]['name'] + '</a>&nbsp;</li>';      
 	}

 	contributors = contributors + '</ul></div>';
